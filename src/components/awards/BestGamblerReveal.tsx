...
interface BestGamblerRevealProps {
  players: Player[];
  onReveal: () => void;
  onContinue: () => void;
...
export function BestGamblerReveal({
  ...
  const gamblingLeaderboard = [...players]
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8">
        ...
        <div className="smash-card rounded-xl p-8 text-center border-[#ffd700]"
          ...
          <p className="text-gray-400 text-lg">
            {hasBestGambler
              ? `${gamblingLeaderboard.length} players placed bets this tournament`
              : 'No one finished with positive gambling profit'}
          </p>
        </div>
        ...
        <Button variant="primary" size="lg" onClick={() => {
          ...
          {hasBestGambler ? 'Reveal Best Gambler' : 'Continue'}
        </Button>
      </div>
    );
  }
